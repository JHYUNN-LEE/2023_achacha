...
def detail_crawling(region, manage_num, manage_subnum, start_date, end_date):
    ...
    try:
        ...
        valORG = findORG[textValIdx+10:textValIdx+18] # ORG_ID값 추출을 위해 인덱스 위치 계산
...
def detailDo():
    ...
    with open(f'{jsonFilename}.json','a') as file:
        dump(detailDictList, file, ensure_ascii=False, indent=4)
    ...
    try:
        ...
        detailDictList = dumps(detailDictList, ensure_ascii=False)
        hdfs.write(f'/user/ubuntu/service_data/{jsonFilename}.json', data=detailDictList, encoding='utf-8', overwrite=True)
